...
// --- Imports LangChain (MODIFIÉS) ---
// On importe HuggingFaceInference depuis @langchain/community
...
// --- Outils LangChain (on utilise la classe HuggingFaceInference) ---
...
// ... (Le reste du code est 100% identique à la version précédente) ...
...
const authenticateJWT = (req, res, next) => { /* ... code inchangé ... */ };
const authenticateAPIKey = async (req, res, next) => { /* ... code inchangé ... */ };
...
app.post('/register', async (req, res) => { /* ... code inchangé ... */ });
app.post('/login', async (req, res) => { /* ... code inchangé ... */ });

// --- ROUTE PROTÉGÉE (Génération de clé API) ---
app.post("/user/api-token", authenticateJWT, async (req, res) => { /* ... code inchangé ... */ });

// --- ROUTE DE L'IA (Protégée par la clé API) ---
app.post('/ask', authenticateAPIKey, async (req, res) => { /* ... code inchangé ... */ });
...
// --- On remet les fonctions complètes pour que le code soit copiable ---
...
authenticateJWT = (req, res, next) => {const authHeader = req.headers.authorization; if (authHeader) {const token = authHeader.split(' ')[1]; jwt.verify(token, process.env.JWT_SECRET, (err, user) => {if (err) return res.sendStatus(403); req.user = user; next();});} else {res.sendStatus(401);}};
authenticateAPIKey = async (req, res, next) => {const apiKey = req.headers['x-api-key']; if (!apiKey) return res.status(401).json({ error: 'Clé API manquante dans le header X-API-Key' }); const conn = await getDbConnection(); const [rows] = await conn.execute('SELECT id FROM users WHERE api_token = ?', [apiKey]); if (rows.length === 0) return res.status(403).json({ error: 'Clé API invalide' }); next();};
...
app.post('/ask', authenticateAPIKey, async (req, res) => {const { question } = req.body; if (!question) return res.status(400).json({ error: 'La question est manquante.' }); try {const answer = await askAI(question); res.json({ answer: answer });} catch (error) {res.status(500).json({ error: "Une erreur interne est survenue." });}});
